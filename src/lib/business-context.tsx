...
export function BusinessProvider({ children }: { children: ReactNode }) {
  ...
  return <BusinessCtx.Provider value={{ data, setData }}>{children}</BusinessCtx.Provider>;
}
...
// Seeded pseudo-random for stable noise across renders
...
export type Cenario = {
  reducaoFaturamento: number; // 0 - 0.3
  atrasoFornecedor: number; // 0 - 15 dias
  comReforma: boolean;
};
...
export function calcularProjecao(d: BusinessData, c: Cenario): Projecao[] {
  ...
  const semanaAtraso = Math.min(11, Math.floor(c.atrasoFornecedor / 7)); // desloca uma saída para frente
  ...
  for (let i = 0; i < 12; i++) {
    const ruido = 1 + (rng() - 0.5) * 0.16; // ±8%
    const entradaSemana = entradaBase * ruido * fatorReducao;

    // Atraso: na semana indicada, subtrair menos; redistribui na próxima
    ...
    // Curva com reforma: aplica acréscimo sobre faturamento semanal
